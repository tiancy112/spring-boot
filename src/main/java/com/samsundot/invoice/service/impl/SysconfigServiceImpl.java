...
import com.samsundot.invoice.mapper.SysConfigMapper;
...
public class SysconfigServiceImpl implements SysConfigService {
	...
	public boolean Import(String fileName, MultipartFile file) throws IOException {
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			// 获得值
			// String id= this.getCellFormatValue((Cell) row.getCell(0));
			String configName = this.getCellFormatValue((Cell) row.getCell(0));
			String configKey = this.getCellFormatValue((Cell) row.getCell(1));
			String configValue = this.getCellFormatValue((Cell) row.getCell(2));
			String status = this.getCellFormatValue((Cell) row.getCell(3));
			String description = this.getCellFormatValue((Cell) row.getCell(4));
			...
			// String configName = row.getCell(1).toString();
			// String configKey = row.getCell(2).toString();
			// String configValue = row.getCell(3).toString();
			...
			//// String createBy = row.getCell(7).getStringCellValue();
			//// Date updateTime = row.getCell(8).getDateCellValue();
			//// String updateBy = row.getCell(9).getStringCellValue();
			//
			SysConfig config = new SysConfig();
			// SysConfig config1=sysConfigMapper.findOne(Integer.parseInt(id));
			// config.setId(Integer.parseInt(id));
			config.setConfigName(configName);
			config.setConfigKey(configKey);
			config.setConfigValue(configValue);
			config.setStatus(Integer.parseInt(status));
			config.setDescription(description);
			config.setCreateTime(new Date());
			config.setCreateBy("admin");// TODO 当前登录人
			...
			// config.setCreateBy(config1.getCreateBy());
			// config.setUpdateBy("admin");//TODO 当前登录人
			// config.setUpdateTime(new Date());
			// }
			list.add(config);
		}
		sysConfigMapper.save(list);
